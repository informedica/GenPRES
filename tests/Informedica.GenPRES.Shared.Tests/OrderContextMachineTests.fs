...
let viewTests =
    testList
        ...
        [
            test "no patient; a seed and the context held: settled" {
                noPatient
                |> OrderContextState.view
                ...
                seeded paracetamol
                |> OrderContextState.view
                |> Expect.equal "the seed, shown while it waits" (OrderContextView.Settled paracetamol)
            ...
            test "the order dialog's context: the selected context, settled or changing as the plan is" {
                ...
                OrderPlanMachine.OrderPlanState.held plan (Some "c-1")
                ...
                OrderPlanMachine.OrderPlanState.changing plan (Some "c-1") (OrderPlanCommand.Recalculate plan) "r-1"
                |> OrderPlanMachine.OrderPlanState.view
                |> OrderContextView.dialog
                |> Expect.equal "changing with the plan" (Some(OrderContextView.Changing inPlan))

                OrderPlanMachine.OrderPlanState.held plan None
                |> OrderPlanMachine.OrderPlanState.view
                |> OrderContextView.dialog
                |> Expect.equal "no selection, no dialog" None

                OrderPlanMachine.OrderPlanState.held plan (Some "c-9")
                |> OrderPlanMachine.OrderPlanState.view
                |> OrderContextView.dialog
