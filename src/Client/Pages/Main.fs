...
//          EmergencyListModel : EmergencyList.Model Option
    ...
    [<ReactComponent>]
    let View(input : {|state : State; dispatch : Msg -> unit|}) =
        Mui.container [
            Views.SideMenu.render ({ Views.SideMenu.Model.Open = true; Views.SideMenu.Items = [] }) (fun _ -> ())
            Html.h1 input.state.Count
            Mui.button [
                prop.text "Increment"
                prop.onClick (fun _ -> input.dispatch Increment)
            ]

            Mui.button [
                prop.text "Decrement"
                prop.onClick (fun _ -> input.dispatch Decrement)
            ]
        ]

    let render state dispatch = View ({| state = state; dispatch = dispatch |})  
