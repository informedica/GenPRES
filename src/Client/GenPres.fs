    ...
    <React.StrictMode>
        <ThemeProvider theme={theme}>
            <Box sx={ {| height="100vh"; ``overflowY``="hidden" |} }>
                ...
                {Pages.Views.Components.SideMenu ({| anchor = "left"; isOpen = model.SideMenuIsOpen; toggle = fun _ -> ToggleMenu |> dispatch |})}
                <Container sx={ {| mt= 5 |} } >
                    <Stack>
                    { Pages.Views.Patient ({| patient = props.patient; dispatch = props.updatePatient |}) }
                    { Pages.Views.EmergencyList ({| interventions = props.bolusMedication |})}
                    </Stack>
                </Container>
            </Box>
        </ThemeProvider>
