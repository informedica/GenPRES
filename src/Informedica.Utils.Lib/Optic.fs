...
/// <summary>
/// Functions to get and set the value a <c>Lens</c> or <c>Prism</c> focuses on.
/// </summary>
/// <remarks>
...
/// SDK 10.0.400 (dotnet/fsharp#20253). <c>Lens</c>/<c>Prism</c> here are the
/// same plain tuple shapes Aether uses, so existing lens/prism *values* defined
/// elsewhere as <c>('a -> 'b) * ('b -> 'a -> 'a)</c> need no changes; only the
/// dispatch functions below replace Aether's <c>Optic.get</c>/<c>Optic.set</c> and the
/// <c>Aether.Operators</c> <c>>-></c>/<c>>?></c> operators.
///
/// Naming convention carried over from Aether: a value whose name ends in <c>_</c>
/// (e.g. <c>min_</c>, <c>DoseRange.Norm_</c>, <c>fst_</c>) is a lens or prism, i.e. a
