...
- `src/Informedica.GenFORM.Lib/DoseRuleLoader.fs` (`getFromGetData`, loading), `DoseRuleData.fs` (parsing) and `DoseRule.fs` (`mapToDoseRule`, `Print.toMarkdown` / `Print.printGenerics`) — the production parser Pass 5 (§6.5) bridges into; the module split is described in each file's header. Resource loading is in `src/Informedica.GenFORM.Lib/Api.fs` and `Resources.fs`.
