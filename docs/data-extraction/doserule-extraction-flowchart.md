...
Every TSV row in the pipeline materializes one full path through a strict hierarchy of facts about a drug. Each pass extends the tree one level deeper; each user step refines the level the previous pass populated. Pass 4 hashes prefixes of the path to assign stable identifiers.
...
| 4 | Pass 1 | S | L3 Route | Extract route, canonicalize to enum (`ORAAL`, `INTRAVENEUS`, …); apply default-Oraal post-process. | `Route` | Pass 1 NuExtract + `PostProcess.defaultOralRoute` |
...
| 6a | Pass 1 | S | (synthesized) | Compose `OriginalText` from `PatientText` + `ScheduleText` (`PatientText: ScheduleText` join). | `OriginalText` | `Tsv.composeOriginalText` |
...
[User validates pass4 — final TSV before downstream typed-emit (TBD)]
   ...
   │         <stem>_pass4 Drive Sheet, overrides the doseRuleData
   │         registry entry to read it, runs the real
   │         DoseRuleLoader.fromData / DoseRule.mapToDoseRule /
   │         addFormLimits, prints + writes <stem>_doserules.md.
   │         Phase5.diagnose replays the pipeline stage-by-stage
   │         and prints the validation warnings per stage.)
   ▼
...
flowchart TD
    ...
    U4 -.optional validation.-> P5["P5  Pass 5 (validation) — GenFORM DoseRule parser bridge<br/>pull latest &lt;stem&gt;_pass4 Drive Sheet;<br/>override the Keys.doseRuleData registry entry with<br/>ofResult (fun () -> Phase5Pure.getDataFromTsv pass4Tsv);<br/>real DoseRuleLoader.fromData / DoseRule.mapToDoseRule / addFormLimits;<br/>render per-generic toMarkdown → stdout + &lt;stem&gt;_doserules.md.<br/>runPhase5Diag = stage-by-stage trace printing the<br/>validation and product warnings per stage"]
...
| **Input** | The `<doseringen>` section of the FTK preparaattekst XML for one generic — passed directly to NuExtract; no curation, no normalization |
...
| **Output** | An array of `(doseType, doseText, text)` per input row. `doseType` is canonicalized client-side against `{once, onceTimed, discontinuous, timed, continuous}`; unknown tokens fold to `"unfinished"` (`Phase2.canonicalizeDoseType`) |
...
| **Hash** | SHA-1 over the tab-joined normalized key, first 6 bytes rendered as 12 lowercase hex chars (~16M before 50% birthday collision; well over the FTK + NKF + future-formularies row count). |
...
> `DoseRuleLoader.getFromGetData` in `src/Informedica.GenFORM.Lib/DoseRuleLoader.fs` — a
> seam that parameterizes the dose-rule data reader so the extraction bridge
...
| **What it does** | Reuses the **production** GenFORM pipeline: takes `Resources.defaultRegistry dataUrlId`, replaces the `Keys.doseRuleData` entry with `ofResult (fun () -> Phase5Pure.getDataFromTsv <pass4Tsv>)`, then `Resources.loadAllResourcesWithRegistry`. The `Keys.doseRules` entry is untouched, so the real route mappings, form routes and products are used and `DoseRuleLoader.fromData` builds the rules; only the dose-rule data source is swapped to the extracted TSV. (`DoseRuleLoader.getFromGetData` is the equivalent adapter for callers that hold a `GetDoseRules`-shaped function.) |
| **`Phase5Pure.getDataFromTsv`** | Mirrors `DoseRuleLoader.getData` — i.e. `DoseRuleData.parseDoseRuleData` over rows read with `File.ReadAllLines` + tab-split instead of the Google sheet. The Pass-4 header is exactly the canonical `DoseRules` column set (`DoseRuleData.headers`), so every column resolves by name, `IsAdult` included. Pure; returns `Result<DoseRuleData[], Message list>`. |
...
| **Diagnostic** (`Phase5.diagnose`) | Replays the pipeline stage-by-stage, printing per-stage survival counts and what each stage drops: (1) raw rows, (2) the `DoseRuleData.validateData` partition (invalid rows become `Warning` messages), (3) `DoseRuleLoader.group` (product candidates per component; row de-duplication warnings), (4) `DoseRule.mapToDoseRule` + `addFormLimits` and `DoseRule.removeEmptyLimits`. In production these warnings are returned as the `Message list` of `fromData` and surfaced through `LoadedResources.Messages`; the diagnostic prints them next to the rows they concern. |
...
| **How to run** | From the script's directory: `echo 'Run.runPhase5 ();;' \| dotnet fsi --use:Informedica.NLP.Lib.fsx` (needs Drive ADC auth). Use `--use:` (not `#load` of the script from a wrapper — FSI does not expose a `#load`ed script's modules and the nested nuget refs trip NU1504). `Run.runPhase5Diag ()` for the stage trace. |
...
Each pass's text output is a strict substring of the previous pass's output (and ultimately of the original `<doseringen>` text). Client-side touches content only via unit canonicalization and numeric conversion (days / grams / m²); the original Dutch tokens are recoverable from the per-generic audit JSON.
...
Per-pass column-fill detail lives in §6.1 / §6.2 / §6.3 / §6.4 ("TSV columns filled" / "still blank" rows). Field-level source of truth: `DoseRuleData` in `src/Informedica.GenFORM.Lib/Types.fs`; Pass 3 Limits target: `DoseLimit` in the same file.
...
- **Downstream typed-emit (TBD, unnamed).** Drop the extraction-only columns (`PatientText`, `CmpBased`); quarantine `unfinished` rows from Pass 2; emit `data/sources/Rules/doserules.tsv`. Note: the `string` → `Gender` DU / `string` → `Informedica.GenUnits.Lib.Units.*` / `float`/`int` → `BigRational` elevation is **already performed by the production parser** (`DoseRuleData.parseDoseRuleData`), which Pass 5 (validation) exercises against the Pass-4 TSV — so this step is column-drop + quarantine, not a separate primitive-validation pass.
- **GenFORM ingest.** Standard pipeline downstream of `doserules.tsv`.

> **Blocking precondition — `IsAdult` matching in GenFORM `.fs` source (partially implemented).** GenFORM now parses and carries the facet: `DoseRuleData.IsAdult` is read from the sheet column (`DoseRuleData.fs`), `PatientCategory.Age` has an `IsAdult` case next to `AbsoluteAge` (`Types.fs`), and `DoseRule.fs` maps between them. What is **not** implemented is the matching guard: `Patient.getAge` (`Patient.fs`) returns an empty age range for the `IsAdult` case, which the age filter treats as "no age restriction", so an adult-only rule would match a patient of any age. Because the extraction pipeline also empties `MinAge`/`MaxAge` when `IsAdult = "x"`, such a row has **no effective age bound** and would violate the safety-by-construction / completeness guarantees in [`core-domain.md`](../domain/core-domain.md). Therefore **no `IsAdult = "x"` row may reach GenFORM ingest** until patient-matching enforces "adult patients only" for such rules (the `TODO` in `Patient.fs`). This is owned by the maintainer per the script-only policy — and is a *different* `.fs` change from the `getFromGetData` seam that Pass 5 (validation) already depends on (see §6.5). **Pass 5 (validation) cannot catch this** — it is structurally blind to `IsAdult` (bridges the legacy parser; see §6.5), so an `IsAdult = "x"` row renders cleanly as age-unbounded with no warning. A green Pass 5 is not evidence such a row is safe to ingest.
...
| `Phase5Pure` / `Phase5` | Pure: `getDataFromTsv` (mirrors `DoseRuleLoader.getData`; local Pass-4 TSV → `Result<DoseRuleData[], Message list>`). Effectful shell (GenFORM parser bridge — no NuExtract): `parseToDoseRules` (replace the `Keys.doseRuleData` entry of `Resources.defaultRegistry` with `ofResult (fun () -> getDataFromTsv <pass4Tsv>)`, then `loadAllResourcesWithRegistry`), `run` (render per-generic via `DoseRule.Print.toMarkdown` + write `<stem>_doserules.md` next to the script), `runFromDrive` (pull latest `<stem>_pass4` Sheet then `run`), `diagnose` / `diagnoseFromDrive` (stage-by-stage replay printing the validation, de-duplication and product warnings per stage). The script must be kept in step with this production API; the registry replaced the earlier `ResourceConfig` record. |
...
- Pass 4: `<generic>.pass4.json` (one entry per output row: `inputRowIndex`, `id`, `grpId`, `sortNo`, `doseType`, `grpFields[]` (the normalized key segments hashed into GrpId), `idFields[]` (the normalized key segments hashed into Id), `generic`)
- Pass 5: no JSON. `Phase5.run` writes a single markdown artifact `<stem>_doserules.md` **next to the script** — the rendered `DoseRule` records, one generic section after another. `Phase5.diagnose` prints its stage trace to stdout only.
...
- `src/Informedica.GenFORM.Lib/Types.fs` — `DoseRuleData` (canonical column source of truth) and `DoseLimit` (Pass 3 Limits target).
- `src/Informedica.GenFORM.Lib/DoseType.fs` — `DoseType.fromString` / `toDescription` (canonical `doseType` enum mirrored by Pass 2).
- `src/Informedica.GenFORM.Lib/DoseRuleLoader.fs` (`getData`, `fromData`, `getFromGetData`), `DoseRuleData.fs` (`headers`, `parseDoseRuleData`, `validateData`) and `DoseRule.fs` (`mapToDoseRule`, `addFormLimits`, `removeEmptyLimits`, `Print.toMarkdown` / `Print.printGenerics`) — the production parser Pass 5 (§6.5) bridges into. The resource registry (`defaultRegistry`, `Keys`, `ofResult`, `loadAllResourcesWithRegistry`) is in `src/Informedica.GenFORM.Lib/Resources.fs`; `Api.fs` exposes it through `IResourceProvider`.
...
- [`docs/domain/genform-free-text-to-operational-rules.md`](../domain/genform-free-text-to-operational-rules.md) §3, §5, §6.1, §6.2, **Appendix C.2** (DoseRule field spec; canonical source for `Gender = male / female` etc.).
