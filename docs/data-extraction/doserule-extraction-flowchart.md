...
- Pass 5: no JSON. `Phase5.run` writes a single markdown artifact `<stem>_doserules.md` **next to the script** (`src/Informedica.NLP.Lib/Scratch/`) — the rendered `DoseRule` records, one generic section after another. It is a generated artifact (untracked under the opt-in `.gitignore`, like `<stem>_pass4.tsv`). `Phase5.diagnose` prints its stage trace to stdout only.
