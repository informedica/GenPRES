...
| 5.11b | Pharmacy mail / electronic hand-off of the TPN letter | **Gap** | — | The print is a paper hand-off only; nothing is sent to the pharmacy electronically. See the former pharmacy-notification use case (UC-GENPRES-003, retired with the integration model) |
...
| 5.17 | Remove an individual component from an infusion (principal / solvent / additional roles) | **Gap** | — | AP2019 removes components inside a solution by check box, refusing the principal component and any solvent a maximum concentration depends on. GenPRES removes whole lines only; the three component roles are not represented. See the component-removal extension in the former nutrition use case (UC-GENPRES-004, retired with the integration model) |
...
| 9.8c | Order file export | **Gap** | — | No order export exists. The nearest intended path is the pharmacy hand-off in the former pharmacy-notification use case (UC-GENPRES-003, retired with the integration model) and 5.11b |
