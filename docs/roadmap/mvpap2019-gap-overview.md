...
| 2.1.7 | PIN enrolment and reset | **Partial.** Enrolment is done: mailed six-digit code, three tries, fifteen minutes; PBKDF2 credential, three wrong PINs, doubling lock-out (`Credential.lockFor`, `:283`); the enrolment form. Reset (uc-06) and the wrong-count decay of Rule 28 are not built | Issue, small: PIN reset and the decay. Rework only if the hospital chooses badge confirmation instead (section 5) | — |
...
| 2.1.11 | Append-only audit store | **Missing** (`Ports.fs:100`: "not built yet") | #516, step 9 of its plan (Rule 46); its hosting is an arrangement (A5) | #516 |
...
|---|---|---|---|---|
| **D2** | Production database engine | [ADR-0007](../adr/0007-session-persistence.md) fixes relational, append-only, an adapter of the session machine, and SQLite as the test and development database, never production (§4); the production engine is deferred to a dated amendment. The two-store split (2.2.2) is settled by §1 | the production run of #516, A5 | before M2 closes |
