...
and it is not shown. A weight measured at the bedside after the reading is the better value;
...
Read before planning; the reading, the Session, the signature and the store are one path, and
...
- **Two patient types.** `Informedica.GenCore.Lib.Patients.Patient` in `GenCORE.Lib/Patient.fs`
  ...
  reads it. Its shape has gaps a reading will meet: `BirthDate` is partial, month and day
  ...
  no name. `GenForm.Patient` in `GenFORM.Lib/Types.fs` is the projection the rules match: an
...
- **The reading.** `ServerApi.Ports.fs` declares `PatientDataPort` as `read: string ->
  GenForm.Patient option`: what the platform hands over is the projection, with an age in
  ...
  `ServerApi.Adapters.fs` where the session port is built, so a reading with an age alone
...
- **The challenge.** `Session.challenge` re-reads the platform and compares the reading with
...
- **The commit.** `Session.commit` writes the version and then sets the Session's patient to
  the challenge's reading, else the plan's patient just signed: `Patient = challenge.Reading |>
  Option.defaultValue version.Plan.Patient`. A bedside weight in the plan is in the version and
  gone from the Session the moment it is signed, when the platform read a different one.
- **The resume.** The session machine's `Resume` answers `SetPatient` with the Session's
  ...
  for every launched patient. The next open of a Session starts from the platform's reading
  as well.
...
- *On the server, at the open and again right after each sign, from the birthdate the reading
...
that turned since the open as it treats a changed reading, a data notice before the challenge,
is left to review; it would reuse the notice as it is.

**What the reading carries.**

- *The reading is the core patient.* **Chosen.** `PatientDataPort.read` answers
  `Patients.Patient option`: the person, with the birthdate as its age and the platform's
  measurements as dated actual weights and heights, and, added to the type, its identity, name
  beside the birthdate. The GenFORM patient the rules match is derived from it at a date by
  one projection, `GenForm.Patient.ofCore`, which becomes the core patient's first use in the
  pipeline and retires its header note. The projection reads the calculation weight and
  height, so whatever writes an actual measurement writes the calculation value too, since
  the type derives nothing. The shape gaps are work of the first step, not left to be found: a
  partial birthdate is read without throwing, and a birthdate without a day or a month makes
  no identified patient; the age value's optional parts become the days the projection needs;
  the department is read.
- *A wrapper beside the GenFORM patient, name, birthdate and dated measurements as a record
  of the server's.* Rejected, with its cost stated: it is the smaller change, since the core
  type carries the gaps above, but it re-invents the person the core type already is, and the
  core type is the domain's own statement of what a patient is. The reviewer of this plan
  recommended the wrapper; the choice for the core type is the project's.
...
The identity is shown in full to the user, who sits at the screen with the patient's record
open in the system that launched GenPRES. It is kept by the Session, its store and the versions,
which is where patient information may be retained. It is never written to a debug or log line.
For the projection that holds by construction, since its printer does not know the fields; for
the core patient it is a discipline, since the type has its own printer and Dto: this plan
allows the name on the core Dto in the three session columns and the versions table, and on no
printer, and step 1 proves the core printer does not write it.
...
- *The reading as read.* **Chosen.** `OpenedSession` keeps the platform's core reading beside
  the projection it opened on, and `challenge` compares a fresh core reading with that, not
  the projections. The projection's age is computed and the reading's is not, so comparing
  projections would report a change for a reading that did not change.
- *Computing the fresh reading's age too, at the same moment, and comparing projections.*
...
- *A measured value the user entered stays for the Session, whatever the platform reads later;
  ...
  **Chosen**, as #976 decided. The value is the user's by rule 3, the notice is the user's to
  ...
  merges instead of replacing, the identity and the age from the reading and the measured
  values kept, on the client and in the plan it challenges; and the commit sets the Session's
  patient to the same merge, the reading's identity and the fresh age with the Session's
  measured values, instead of the reading alone.
...
- A platform reading is the core patient, `Patients.Patient`, its identity added to the type:
  the person with a birthdate and dated measurements. The Session keeps it as read, and
  derives the GenFORM patient from it at a date with one projection,
  `GenForm.Patient.ofCore`, the estimate applied over the projection where the port's wrap
  applies it today.
- At the open, an identified reading's age is `Patient.SetGet.getAgeValue now` over its
  birthdate, the reading's own age value ignored when it has both. That age is the Session's up
  to and including its next sign. The challenge compares core readings.
...
- The accept of a data notice and the commit both merge: the reading's identity and age, the
...
- The store's three patient columns and the versions table change by expand and contract
  under a new structure version, the core reading as the core patient's Dto.
...
step must close before anything reads it. There is no live platform adapter, so the reading is
...
1. **The reading as the core patient, as a script.** A script in
   `src/Informedica.GenPRES.Server/Scripts/` shadows `Patients.Patient`, `GenForm.Patient`,
   `Ports`, `StubPatientData` and `Mappers.Session`: `PatientIdentity`, name and birthdate, on
   the core patient; the partial birthdate read without throwing, and none of it an identity
   when the day or the month is missing; `GenForm.Patient.ofCore now`, the projection at a
   date, the age from `Patient.SetGet.getAgeValue now` turned into days, the measures from the
   calculation values, the department read, the rest field for field; the port's `read`
   answering a core patient; the stub building one for its fixed patient, a constant birthdate
   and a name, and none for `no-data`; `OpenedSession` carrying the core patient as read
   beside the projection; the contract's `PatientIdentity` in `Shared/Types.fs`, the birthdate
   as three integers, on `PatientContext`. Proved: the projection of the stub's patient at a
   fixed date is the patient the stub answers today, field for field; a reading round-trips to
   the client's context with its name and birthdate; a `no-data` launch has none; a reading
   with a partial birthdate opens anonymous; the core printer and the projection's printer
   write neither the name nor the birthdate.
2. **The reading migrated**, in two pull requests. First the identity and the reading into
   ...
   both versions, per ADR-0008 section 6, with the reading kept beside the projection.
3. **The age computed at the open, as a script.** `sessionPatient` shadowed: the projection at
   `now` of the reading, so an identified reading opens on the age its birthdate gives and the
   reading's own age value is ignored; `now` is the clock the Session already takes; the
   estimate applied over the projection here, and the port's wrap in `ServerApi.Adapters.fs`
   gone. `challenge` shadowed to compare the fresh core reading with the one the Session kept,
   not the projections. Proved: a reading with the stub's constant birthdate opens on the age
   that birthdate gives at the clock's date whatever age value it carried; a reading without a
   birthdate opens as today; the estimate is the one for the computed age; a challenge over an
   unchanged reading is a challenge, not a data notice, and one over a changed reading is a
   notice.
...
8. **The version with the identity, and the age after the sign, as a script.**
   ...
   the reading's identity with the age computed again at `now`, the projection and the
   estimate over it, and the Session's measured values, instead of the reading alone, and
   ...
   that opens from the head, the platform answering none, opens with the head's identity;
   after a commit with the clock a day on, the Session's age is a day more and the version's
   is not; after a commit with the platform reading another weight, the Session's weight is
...
10. **The title bar and the panel.** `Components/TitleBar.fs` shows the identity beside the
    ...
    merges the notice's data into the draft, identity and age from the reading and the
...
- Steps 3 and 4 also: a stub launch as the prescriber, a challenge requested without any change
  on the platform, and the answer read: a challenge, not a data notice; the same at a clock
...
- **A day that turned without a sign.** Whether the challenge should answer a data notice
  when the age at that moment differs from the Session's, as it does for a changed reading,
