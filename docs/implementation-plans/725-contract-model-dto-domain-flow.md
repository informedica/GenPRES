...
| 2.3, the nutrition rule sets | #750 | `Server/Scripts/NutritionRuleSets.fsx`: the five sets as `NutritionRuleSet` values with their labels and `all` for the composition root, copied from the nutrition service, whose dose types were empty in every set and have no counterpart; four tests pin each set against the service's copy and the label the client shows, the golden one to go with the copy in 4.3 |
