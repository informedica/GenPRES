...
| 1.2a, the measured flags | #736 | `GenFORM.Lib/Scripts/PatientMeasured.fsx`: `WeightMeasured` and `HeightMeasured` on the shadow `Patient`, both set on the empty patient, the five migration sites listed, four tests for `PatientTests`; migrated to source in #738, stacked on #737 |
| 1.2b, validate and the patient Dto | #739 | `GenFORM.Lib/Scripts/PatientDto.fsx`: `PatientError`, `Patient.validate` reading the flags, `Patient.Dto` (ages in days, weight in kilograms, height in centimetres, gender, access and renal function as strings), `Gender.tryFromString`, string modules for access and renal function; fourteen tests (validate, L1, L2, refusals, string forms); migrated to source in #741 |
| 1.3, the scenario Dtos and the canonical form | #740 | `GenORDER.Lib/Scripts/ScenarioDto.fsx`: `DtoError`, `Filter.Dto`, `OrderScenario.Dto` (text blocks as kind and text, the order as `Order.Dto.Dto`), the canonical serializer (Newtonsoft, no whitespace, `n/d`, options as value or null, a resolver that reads the constructor-only Dto classes back); nineteen tests, `Order.Dto` L1 holds on the three test scenarios; migrated to source in #742, stacked on #741: `OrderScenario.Dto` in `Api.fs`'s existing `OrderScenario` module, the rest in the new `OrderPlan.fs`, GenFORM's `Filter` module named in full in the services file |
| 1.4, the order plan Dtos | #743 | `GenORDER.Lib/Scripts/OrderPlanDto.fsx`: `Totals.Dto`, `OrderContext.Dto`, `PlanContext.Dto`, `OrderPlan.Dto`, `Signer.Dto`, `OrderPlanVersion.Dto`, the category as a string, three more `DtoError` cases; eighteen tests, the stored root reads back from its canonical form; migrated to source in #744: the Dto helpers file renamed `Dtos.fs`, `OrderContext.Dto` in `Api.fs`, a new `OrderPlan.fs` after `Api.fs` for the order plan modules; Phase 1 complete |
| 2.1, the order plan rules | #746 | issue #745; `GenORDER.Lib/Scripts/OrderPlanRules.fsx`: `OrderPlan.create`, `orders`, `filtered`, `nutritionContexts`, `holds`, `admits`, `updateContext ruleSets id`, `removeOrderContext(s)` with the feeding cascade, `addOrderContext newId` on a `PlanContext` workbench, so its intake comes along; `PlanContext.create`, `nutritionCategory`, `contribution`; `NutritionRuleSet.narrow` and `tryFind`, `Totals.empty`; the refusals as an `OrderPlanError` union, the words for each left to the server; twelve tests for a new `OrderPlanTests.fs`. `narrow` moved here from 2.2 since `updateContext` needs it; migrated to source in #747: `OrderPlanError` in `Types.fs`, `Totals.empty`, `OrderContext.contribution` in `Api.fs`, the rules in `OrderPlan.fs` before each module's Dto, the tests as a new `OrderPlanTests.fs` |
| 2.2, the evaluation around a plan context | #748 | `GenORDER.Lib/Scripts/OrderPlanEvaluate.fsx`: `Filter.reconcile fresh held` (the mapper's narrowing, verbatim, pure over two filters), `OrderContext.reconcile logger provider` and `intake totalsData`, `PlanContext.evaluate logger provider totalsData cmd` as the pipeline (reconcile, the command, evaluate, the intake recorded), `NutritionRuleSet.discover evaluate`; six tests for `OrderPlanTests.fs`, two behaviours of the moved narrowing pinned (a fresh dose-type list of one entry is taken as it is; nothing held keeps the held, empty, dose types); a run on the demo data yields one scenario with its intake on the second pass. `evaluate` reconciles itself, since the server always does before an evaluation; migrated to source in #749: `Filter.reconcile` in `Dtos.fs`, `OrderContext.reconcile` and `intake` in `Api.fs` after `evaluate`, `PlanContext.evaluate` and `NutritionRuleSet.discover` in `OrderPlan.fs`, the tests as a second list in `OrderPlanTests.fs` |
| 2.3, the nutrition rule sets | #750 | `Server/Scripts/NutritionRuleSets.fsx`: the five sets as `NutritionRuleSet` values with their labels and `all` for the composition root, copied from the nutrition service, whose dose types were empty in every set and have no counterpart; four tests pin each set against the service's copy and the label the client shows, the golden one to go with the copy in 4.3; migrated to source in #751: `ServerApi.NutritionRuleSets.fs` after the ports file, `NutritionRuleSetsTests.fs` in the server tests; Phase 2 complete |
| 3.0, the enteral tube | #753 | issue #752; `GenFORM.Lib/Scripts/EnteralTube.fsx`: `AccessDevice` shadowed with `EnteralTube`, next to the four sites the case touches (`VenousAccess.check` unchanged, its arms are wildcards; the Dto string form `enteral-tube`; the rule-side `Location` form `TUBE`; a solution-rule heading); four tests pin the check on every venous row, a tube alone matching no venous rule, the round trips; a run of the library's `filterPatient` for a PVL patient over three categories gives the rows the migrated test pins; migrated to source in #754: the case in `Types.fs`, the string forms in `Patient.fs` and `Product.fs`, the heading in `SolutionRule.fs`, an `AccessDeviceTests` module in the GenFORM tests; no incomplete-match warning, so those are every site |
| 3.1, the patient mapper | #755 | `Server/Scripts/MappersPatient.fsx`: `ofModel` (total, the tube mapped, an empty department kept empty, the post-menstrual age the contract model's own), `toModel`, `parse` (the two steps, the server's words, the weight-and-height gate kept); seven tests: the stub patient round-trips, L3 for a full and an estimated patient and what falls outside it, every field survives, the refusals; the golden: a patient without a department matched the ICK and any-department categories on the old mapper and matches every department's on the new one, a department given as before. Migrated to source in #756: `git mv` to `ServerApi.Mappers.Patient.fs`, the module keeping its name, `patient`/`patientOption` kept until Phase 4, the fitness allowance for the old name removed (the allow-list shrinks by one), `MappersPatientTests.fs` in the server tests |
| 3.2, the order context mapper | #757 | `Server/Scripts/MappersOrderContext.fsx`: `ofModel : OrderContext -> PlanContext.Dto` (total; the id, category and intake the plan gave the context; scenarios through the order mapper, numbered by place; text blocks rendered back to marked-up text, the inverse of `parseTextItem` that nothing had), `toModel demo` from the Dto alone, `Command.toDomain` (the service's table), the category and the dose type both ways; five tests: L3 on a context with a test-scenario order, blank text items and the scenario number outside it, the Dto parsing through `PlanContext.Dto.fromDto`, the text inverse, every one of the 24 verbs. Migrated to source in #758: `ServerApi.Mappers.OrderContext.fs` after the patient mapper, one `OrderContextMapper` module with the text rendering, the category and the command table nested in it (a top-level `OrderCategory` module would shadow the contract type across the server), `MappersOrderContextTests.fs` with `Scenarios.fs` linked into the server tests |
| 3.3, the order plan and session mappers | #759 | `Server/Scripts/MappersOrderPlan.fsx`: `OrderPlanMapper.ofModel` (total, contexts through the order context mapper, categories as strings) and `toModel demo`, `nutritionCategory`; `SessionMapper.ofSigned` / `toSigned demo` (the version's Dto, the signer without a role since only a Prescriber signs, the filter empty and the totals none since the wire's signed plan carries neither), `noticeData` / `notice`, `opened` from the parts the session state holds in Phase 5; eight tests: L3 on a two-context plan and on a signed plan, what falls outside it, both Dtos parsing, every nutrition category both ways, the notice, the opened session. The scripts bootstrap now loads the 3.2 mapper. Migrated to source in #760: `ServerApi.Mappers.OrderPlan.fs` and `ServerApi.Mappers.Session.fs` after the order context mapper, `MappersOrderPlanTests.fs` in the server tests |
| 3.4, the mappers file renamed | #761 | `git mv ServerApi.Mappers.fs ServerApi.Mappers.Order.fs`, the module and its callers unchanged, the two script bootstraps following; no deletion: every part the new mappers duplicate is still called, by the services until Phase 4, by the patient golden test, and by the new mappers themselves (the dose type both ways, the markup parser), so the deletion moves to 4.3 with the services that call them; a refactor straight to source, a rename having nothing to prototype. Phase 3 complete |
| 4.0, the test preparation | #763 | issue #762; two builders in the server tests' stub adapters, `portContext` and `portPlan`, the identity today, through which every context and plan a stub port answers passes: the two stub factories, the inline port of the navigate test, the `openWith` echo of the dispatch test; far smaller than estimated, the other seams being in place (`planOf`, `signedBy`, the echo ports answering their argument) and the tests over `OrderPlanService` going with it in 4.3; tests only, straight to source |
| 4.1, the order context switch-over | #764 | `Server/Scripts/OrderContextSwitch.fsx`: `OrderContextPort.evaluate` on the domain's verb and a `PlanContext`; `OrderContextService.evaluate` as the `PlanContext.evaluate` pipeline, the errors in the server's words; `processCmd` with the patient gate kept, `ofModel` then `fromDto` in, `toDto` then `toModel` out with the environment's demo flag, `setDemoVersion` gone; `evaluateModel`, the contract model in and out over the same pipeline, for the order plan service until 4.2, so one evaluation path remains and the old mapping is unused; `DtoError.words`; five tests; the old path and the new run on the demo data over three passes give the same answers but for the minted order id and the clock; migrated to source in #765: the port and `AppEnv.demo` in `Ports.fs`, the service in `Services.fs` with the plan service taking the evaluation as a function, the adapters and `Server.fs` wiring the flag, the handler, `words` in the order context mapper, `OrderContextSwitchTests.fs`, the totals tests' "+1 hack" gone with the incidental read it counted |
| 4.1, the department | #766 | the walkthrough of #765 found the switch-over faithful but every patient without a department unable to prescribe: GenORDER's `getRules` asks for a department beside a weight and a height and without one answers a context made afresh and no rules, which the mapper's `"ICK"` default had hidden and step 3.1 removed. Decided: the domain accepts the absent department, a patient without one taking the rules of every department (`GenORDER.Lib/Scripts/DepartmentAny.fsx`, the guard gone from the match; three tests on the demo data, where such a patient gets what an `"ICK"` patient gets); #765 merged ahead of it, so master prescribes for a patient without a department only once this lands; migrated to source in #767: the guard gone from `getRules` with a doc comment, three tests over a provider holding no rules pinning the branch (the context kept as held with or without a department, made afresh only without a weight and a height) |
| 4.2, the order plan switch-over | #768 | `Server/Scripts/OrderPlanSwitch.fsx`: `OrderPlanPort` on the domain's order plan, `navigate` with the verb and a `PlanContext`; the adapter over the rules of 2.1 and 2.2 with the rule sets of 2.3 and the provider's totals, `OrderPlan.recalculate` added for GenORDER, `newOrderContext` as admit, a workbench with the category's indications and generics, and `discover` over the evaluation; the handler parsing the plan, the context and the patient through the mappers and mapping out with the demo flag; `OrderPlanError.words` with the category's label from the rule sets; four tests; on the demo data the old port and the new offer the same nutrition workbench for every category (the demo sheet holds few nutrition rules). The scripts bootstrap now loads the rule sets file; migrated to source in #769: `OrderPlan.recalculate` in GenORDER with a test over a solved infusion and the row filter, the port in `Ports.fs`, `words` in the order plan mapper, the adapter over the domain rules with the old contract-typed evaluation gone from `Adapters.fs`, the handler with `parsePlan` and `parseContexts`, `OrderPlanSwitchTests.fs`, the stub plan port and the totals tests on domain values |
| 4.3, the old services deleted | #771 | `OrderPlanService`, `NutritionPlanService`, `OrderService` and `OrderContextService.evaluateModel` gone from `Services.fs`, and with them `mapFromShared`, `mapToShared`, `mapTextBlock` and `mapToTotals` from the old mapper (`mapFromSharedPatient` and the dose type mapping stay: the formulary service and the new mappers call them); the tests over the plan service go, the plan rules being pinned in GenORDER since 2.1 and 2.2; the golden test of the rule sets against the service's copy goes with the copy; the `Services.fs` allowance stays, reworded: the formulary and parenteralia services still name the contract model, and the order context parse; a deletion straight to source, nothing to prototype |
| 4.4, the ports and adapters cleaned up | #772 | `Ports.fs` names the domain through three module abbreviations (`GenOrder`, `GenOrderContext`, `GenForm`) where it wrote the full path on every line, the contract model staying what an unqualified name means there; `Adapters.fs` opens the domain after the contract model so that the two ports on domain values read unqualified; the fitness allowances for `Adapters.fs` and `StubAdapters.fs` reworded to name only what still takes the contract model: the formulary, interaction and admin ports (their own issue), the session and patient data ports and their stubs (Phase 5); a refactor straight to source. Phase 4 complete |
| 5.0, the session test preparation | #774 | issue #773; builders in the server tests' session suite through which every session state literal passes: `versionOf` and `signedAt` for a signed version (four local `signedBy` copies and three inline literals now one line each), `challengeOf`, `noticeOf`, `sessionOf` (four local `session` copies), `storedOf`, `recordsOf` and `versionsOf` for what the records hold and what a test reads back, so that the type swap of 5.1 and the session record of 5.2 touch the builders only; tests only, straight to source |
| 5.1, the session store on domain values | #775 | `Server/Scripts/SessionStore.fsx`: `StoredVersion` (readable, or unreadable by its identity and the reason), the records on it, the head the newest whatever its case, a sign refused against an unreadable head whatever base the client names; a challenge holding the digest of the plan over its canonical form (SHA-256 of the store's serialization of `toDto`), never the plan; `Signature` and `SigningOutcome` on domain values for the signing half of the session port; `commit` pure, returning the write as a `Persist` value, the adapter's `submitWith` running it and assigning the state only when it landed, `StoreFailed` else, a conflict answered as a stale sign against the version that won; the handler parsing the plan of a challenge and of a submission and refusing one the domain does not read before the port, mapping the answer out with the demo flag; `toSigned` threaded where the service writes the head into the contract model's session record, until 5.2; ten tests, the plan's list. Deviations: the notice's data stays on the contract model until the patient data port moves in 5.2; two refusals join the contract model at migration, `StoreFailed` and `PlanUnreadable`; opening an unreadable version leaves the Session as it is until 5.2 says it cannot be shown; migrated to source in #776: `StoreFailed` and `PlanUnreadable` in the contract model with their sentences in the client and the localization script, `Signature` and `SigningOutcome` in `Ports.fs` with the session port's signing half on them, `StoredVersion`, `Persist` and `StoreOutcome` in `Session.fs` with `headOf`, `blockedBy`, `unreadableHead`, `openVersion`, `challenge` and `commit` over them and `toSigned` threaded through the open, the stub's `digest`, `persistNothing` and `submitWith`, the handler, `SessionStoreTests.fs`; the session suites sign a real paracetamol order and build their challenges from a digest, the 5.0 builders parsing what they store |
| 5.2, the session record on domain values | #777 | `Server/Scripts/SessionOpened.fsx`: `OpenedSession`, what the store holds of an open Session on domain values with the head as a `StoredVersion` whatever its case; `SessionMapper.toOpened` making what the client keeps, a readable head as the signed order plan on the wire and an unreadable one as none, the patient as the contract model, the demo flag the server's; the patient data port on the domain's patient, the stub parsing its reading so that a reading that is no patient is no reading at the adapter; `sessionPatient` over the reading, else the readable head's patient, else none; the open holding the head whatever its case and opened with a readable one only; four tests. `SessionEnding.Unreadable` and its gate sentence join at migration, a contract model change a script cannot make; the stub never appends it; migrated to source in #778: `StoredVersion` and `OpenedSession` in `Ports.fs` with the launch, PIN, lookup and version answers on it, the patient data port and the notice's data on the domain's patient, `toOpened` in the session mapper, the session record and the notice and challenge on domain values in `Session.fs` with `toSigned` gone from the open, the reopen and the commit, the stub parsing its reading, the three handlers mapping with the demo flag, `SessionEnding.Unreadable` with its sentence in the client and the localization script; the 5.0 builders parse their patients and the session suites assert on the stored head |
| 5.3, the decision accepted | #779 | ADR-0007 § 3 to Accepted, dated to the session service holding what it says, with the ADR's status line naming it as ADR-0000 asks; plan 516's `order_plan` schema lines were already in place from Phase 0 and 1.3 (`plan` as `OrderPlanVersion.Dto` JSON under `json_version`, the identity columns authoritative, the upgrade-on-load rule, the `unreadable` ending), so the plan gains the digest's form (SHA-256 over the canonical serialization, as the stub computes it) and its steps 3, 4 and 6 record their gates as met by #776 and #778. Deviation: no changelog block, since a `docs` commit never renders in the changelog (DEVELOPMENT.md); the user-visible changes of Phase 5 ride on the `feat` commits of #776 and #778. Phase 5 complete |
| 6.1, the filter sync helpers to the client | #781 | issue #780; the four `sync*` helpers leave `Shared.Models` for `Client/FilterSync.fs`, a module of their own, UI state kept in step between the prescribing filter, the formulary and the parenteralia pages and no rule of the domain; `App.fs` calls them there; a move straight to source, the Fable build the check (the module compiles to its own file and the app imports it) |
| 6.2, the nutrition button rule named | #782 | `Shared.Models.OrderPlan.mayAdd`, the rule the nutrition page's buttons follow, extracted from the view (one context per category, a supplement under a feeding, an electrolyte or glucose line any number of times) with a doc comment naming the domain's `admits` it copies; the page's five buttons ask it, the electrolyte one too; every other client copy of a domain rule carries a doc comment naming the domain function it mirrors (`nutritionCategory`, `contribution`, `nutritionContexts`, `orders`, `filtered`); three tests on `mayAdd` in the shared tests; the agreement of 6.3 pins each copy to the domain; straight to source, the Fable build the check |
| 6.3, the agreement test | #783 | `tests/Informedica.GenPRES.Server.Tests/AgreementTests.fs`: three FsCheck properties over plans of up to four contexts, each a drug or one of the five nutrition categories, holding none, one or two paracetamol orders, kept by the row filter or not; for every such plan the client's `orders`, `filtered` and `nutritionContexts` answer the domain's over the plan parsed at the boundary, `contribution` and `nutritionCategory` agree context by context, and `mayAdd` says yes exactly where `admits` does; the doc comments of the copies name the test; tests only, straight to source. Phase 6 complete, and with it the plan: every contract model boundary maps in two steps to a domain Dto, every port takes domain values, the records and the challenges are the domain's, and the client's copies are pinned |
