...
=======
| 1.2a, the measured flags | #736 | `GenFORM.Lib/Scripts/PatientMeasured.fsx`: `WeightMeasured` and `HeightMeasured` on the shadow `Patient`, both set on the empty patient, the five migration sites listed, four tests for `PatientTests`; migration by the maintainer |
>>>>>>> feat/733-order-plan-types-source
