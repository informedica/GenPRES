...
| 3.0, the enteral tube | #753 | issue #752; `GenFORM.Lib/Scripts/EnteralTube.fsx`: `AccessDevice` shadowed with `EnteralTube`, next to the four sites the case touches (`VenousAccess.check` unchanged, its arms are wildcards; the Dto string form `enteral-tube`; the rule-side `Location` form `TUBE`; a solution-rule heading); four tests pin the check on every venous row, a tube alone matching no venous rule, the round trips; a run of the library's `filterPatient` for a PVL patient over three categories gives the rows the migrated test pins |
