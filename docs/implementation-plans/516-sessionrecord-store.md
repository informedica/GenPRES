...
| 6c, prototype | #PR | `Scripts/SessionSlice.fsx`: the `Slice` a request is keyed by and the `SessionStore` over it; the SQL loader per slice; the port taking a slice instead of a patient, with the login's rows loaded between `redeem` and `openAfterRedeem`; four tests through the port over a file. Two findings: a load replaces what the state held for the rows it read, never merges, else a Session ended elsewhere survives in the memory of the server that opened it; and a launch that opened a Session must answer with it even once that Session has ended, which `loadSession` cannot give, so the rows are read again as an `OpenedSession` |
