...
| 6b, prototype | #805 | `Sql/002-session.sql` (migration 2, the seven tables) and `Scripts/SqlSessions.fsx`: the patient's structure version and its read; `loadLaunch` by nonce or `state`, absent past the lifetime; `loadSession` with the newest opened-with and heartbeat, the head through a `headOf`, the endings (a newer row of the login the supersession, a closed Session gone, an acknowledged ending silent, an unreadable patient ending it); `runWrites`, a request's writes in one transaction, a conflict answered with the head; ten tests |
| 6b, migrate 1 of 3 | #806 | `SqlSessions`: `ms`/`at`, `refusalOf` over the `Session.refusalWord` the client reads, `roleWord`/`roleOf`, the reading helpers, and `loadLaunch` by nonce or `state`, absent past the expiry. The prototype is 290 source lines of loader and writer, so it lands in three PRs: the launch rows, the session rows, the writer. `SqlSessionTests.fs`, six tests, inserting the launch rows themselves until the writer lands |
| 6b, migrate 2 of 3 | #807 | the patient's structure version and its read; `loadOpenedWith`, `loadSeen`, `loadEnding` (a newer row of the login the supersession, read whatever became of it, a closed Session gone, an acknowledged ending silent) and `loadSession` with the head through a `headOf`. What this release cannot read ends the Session: a patient under a newer structure version or one that does not parse, and a Role word `roleOf` does not know. Eight tests |
| 6b, migrate 3 of 3 | #808 | `patientJsonWritten` and the patient's JSON; `endingWord`; `run`, the rows of every `Persist` case; `runWrites`, a request's writes in one transaction, a conflict answered with the head and any other failure with its reason. `SqlDatabase.insertVersion` takes the connection and transaction, so the record's path and a request's share one `order_plan` insert and the `isSameNumber` test. Seven tests, written through `runWrites` and read back through the loader; the prototype script removed. 6b done; 6c next |
...
| 7, prototype | #813 | `Sql/003-credentials.sql` (migration 3: `credential_event` carrying the credential whole per event, `confirmation_code`/`code_try`/`code_spent`, `enrolment`/`enrolment_dropped`) and `Scripts/SessionCredentials.fsx`: the loaders, the slice an attempt names (its person's other attempts, their code and credential), the writer, and the seed written once per login. Six tests. `Session.Persist` has no case for these rows, so the writes are drafted as a `CredentialWrite`; the first migrate makes them `Persist` cases the machine returns |
| 7, migrate 1 of 3 | #814 | `Persist` gains `WriteCredential`, `WriteCode`, `CountCodeTry`, `SpendCode`, `WriteEnrolment`, `DropEnrolmentWrite` and `DropEnrolmentsOf`; the suspension, `supplyPin`, `dropEnrolment` and `commit` return them, and `SqlSessions.run` writes the rows. A second launch of a person writes no second code; the PIN set returns its writes whatever the launch comes to, since the PIN stands even when the active patient moved on. Four tests, and the `dropEnrolment` and `commit` tests assert their writes. 200 changed source lines. Nothing reads the rows yet |
| 7, migrate 2 of 3 | #815 | `loadCredential`, `loadCode`, `loadEnrolments`, `withEnrolment` (an attempt brings its person's other attempts, their code and their credential) and `withCredential`; `withSession` brings the credential of the person the Session belongs to; `SqlSessions.seed`, called by the composition at start-up, since the machine now reads credentials from the rows and the seeded Prescribers would otherwise have none. A PIN set by enrolment, the wrong-PIN count and the lock survive a restart. Five tests |
...
| 9, prototype | #820 | `Sql/005-audit.sql` (migration 5, append-only and never dropped) and `Scripts/SessionAudit.fsx`: the entries derived from the writes a request already returned, rather than an `AppendAudit` case the members return, so an audit cannot describe an act that did not land nor be forgotten at a call site; every write matched, the ones another write audits saying so. Put to the maintainer for a decision. Five tests |
| 9, migrate | #821 | `SqlSessions.auditOf` and `appendAudit` in `ServerApi.SqlAdapters.fs`: the entries of a request written inside the transaction that carries its writes, so an entry describes an act that landed and a failed request leaves none. The derived design of #820 accepted by its merge. The words taken from the store's own vocabulary (`endingWord`, `refusalWord`, `refusalRow`), so the audit and the rows cannot spell one fact two ways. A dead expression in the session loader dropped with it. Six tests |
| 9, docs | #822 | DEVELOPMENT.md and the uc documents in their final wording: the SQLite store keeps everything a Session stands on, the notices, challenges and answers included, and nothing is ever deleted — a row past its lifetime loads as absent. Two walkthrough steps added, a signature finished across a restart (pinned by the cross-server challenge test) and reading the audit table back. uc-01's store row, uc-08's restart paragraph and uc-06's PIN reset corrected; the audit rows of uc-01, uc-02 and uc-03 say it is written on SQLite alone and cannot be read back yet. uc-04's line on a store that decides the race across more than one server stays until the production engine |
| 10, the follow-up issues | no commit | The store: [#823](https://github.com/informedica/GenPRES/issues/823) the production engine, the ADR-0007 amendment, which the rest waits on; [#824](https://github.com/informedica/GenPRES/issues/824) a reader for the audit and what its retention is; [#825](https://github.com/informedica/GenPRES/issues/825) the idle and absolute lifetimes and the request that ends its own; [#826](https://github.com/informedica/GenPRES/issues/826) a bound on clock skew; [#827](https://github.com/informedica/GenPRES/issues/827) the signed request against the key the Launch carried; [#828](https://github.com/informedica/GenPRES/issues/828) a session domain free of the contract types; [#829](https://github.com/informedica/GenPRES/issues/829) an anonymous open and the bound of Rule 14. Plan 725's trailing phases, each its own issue as that plan decided: [#830](https://github.com/informedica/GenPRES/issues/830) O1, [#831](https://github.com/informedica/GenPRES/issues/831) O2, [#832](https://github.com/informedica/GenPRES/issues/832) O5; O3 and O4 were notes only there and are not filed. Not filed because it already stood: what production exposes, [#580](https://github.com/informedica/GenPRES/issues/580), open and not folded into #823 — #823 chooses the store's engine, #580 decides what production exposes and with it that production requires a store at all; until it, production keeps `Adapters.sessionDisabled` and the guard refusing `GENPRES_DB_CONNECTION`. Not filed, being the maintainer's: the three Dutch rows of the localization workbook |
