...
- `OrderContext` gains `Id: string` and `Category: OrderCategory`. A workbench not in the plan
  ...
  the generic for a drug. The removable flag is dropped: every context was removable.
...
- The plan's `Patient` is the patient the plan is computed for and signed on. Each context also
  ...
  or added under the plan's patient; a patient change goes through `ChangePatient`, below.
...
- `changePatient`: the plan's patient replaced and every context re-evaluated from its filter
  ...
  and age; a stepped value that no longer fits is lost, which the page says. Chosen over
  refusing the change while the plan holds orders, because a weight edit during a session is
  ordinary on the ward. A context whose re-evaluation fails is the answer, as a failed
  `Navigate` is today.
...
- The patient panel: an edit while the plan holds contexts sends `ChangePatient`.
...
- Edit the patient's weight with orders in the plan: one `ChangePatient`, every order rebuilt
  for the new weight, the page says what was lost.
