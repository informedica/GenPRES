...
| Threat | Today (2026-09-22) | Mitigation | Likelihood | Impact | Tracking |
|---|---|---|---|---|---|
| A Prescriber denies having signed an order plan version | Guarantee 4 declines to claim non-repudiation and states what would earn it: a fresh hospital sign-on over the SigningChallenge digest, kept with the plan (Open Question 3). The audit (Rule 46) records every launch, Session opening and ending, Submission committed or refused, failed PIN entry and PIN change, in the same transaction as the act | Audit built, demo only, on the SQLite store: the in-memory store audits nothing and nothing reads the table back yet (#824). Non-repudiation designed, not built | not rated | not rated | Integration design, Open Question 3; ADR-0007 |
