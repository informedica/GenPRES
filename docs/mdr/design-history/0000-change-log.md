...
| Date | ADR | Summary |
|------|-----|---------|
| 2026-09-05 | — | Per-domain `MailboxProcessor` agents removed from the server composition (`ServerApi.AgentAdapters` deleted; former ADR-0008, itself removed in `5f930887`). Ports are served by the direct `ServerApi.Adapters`; `Informedica.Agents.Lib` is retained for logging only. See issue #420 |
| 2026-09-01 | [ADR-0021](0021-build-system-versioning-and-release.md) | Amended: `publish-docker-image` now publishes to Docker Hub (`docker.io/informedica/genpres`) instead of GHCR, now that the `informedica` Docker Hub org (Docker Team plan) exists. Auth is Docker Hub OIDC via `docker/login-action` (no stored credential), scoped to a `docker-publish` GitHub environment; adds `id-token: write`, drops `packages: write`. See issue #459 |
| 2026-08-28 | [ADR-0020](0020-fhir-r4-integration.md) | Superseded. The `Informedica.FHIR.Lib` prototype the ADR is built on was deleted in `ad7bdd17` (2026-07-16) — the project was never referenced by `GenPRES.sln`, so never compiled. The ADR is retained as the record of a design chosen and abandoned; the `Hl7.Fhir.R4` dependency it names was never added. Documentation referencing the FHIR, DataPlatform, HIXConnect and OTS libraries and their test projects corrected in the same pass |
