...
Production is an explicit opt-in at container runtime and needs all four of `GENPRES_PROD=1`, the proprietary `GENPRES_URL_ID`, a `GENPRES_PASSWORD` of at least 16 characters (without one the server starts with admin operations disabled and warns, issue #590; a shorter one refuses to start), and a bind mount of the host's `data/cache` onto `/app/data/cache` — production reads `*.cache`, and the image ships only the `*.demo` files. Neither the proprietary URL ID nor the password is baked into the image; inject both at runtime, ideally via a Docker / Kubernetes secret. The repo-root `compose.yaml` wires all of this from `.env`: `docker compose pull && docker compose up -d`.
